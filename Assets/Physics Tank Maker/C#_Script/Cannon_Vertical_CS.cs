...
public class Cannon_Vertical_CS : MonoBehaviour {
	...
	void Start_Tracking () { // This function is called from "Turret_Horizontal".
		Tracking_Flag = true ;
		Moving_Flag = true ;
		if ( Input_Type == 10 ) { // AI
			Random_Offset () ;
	...
	void Stop_Tracking () { // This function is called from "Turret_Horizontal".
		Tracking_Flag = false ;
	}

	void Auto_Turn () { // Calculate the angle and rate, and rotate the cannon.
		// Calculate the angle.
		if ( Tracking_Flag ) { // Tracking now.
			...
			if ( Mathf.Abs ( Target_Angle ) < 0.01f ) { // Cannon returned to the initial position.
	...
	float Auto_Angle () { // Calculate the proper angle.
		float Temp_Angle ;
	...
	float Manual_Angle () { // Simply turn toward the target.
		float Temp_Angle ;
		Vector3 Target_Pos = Turret_Horizontal_Script.Target_Pos ;
	...
	void AI_Input () {
		...
		if ( AI_Script.Direct_Fire ) { // Direct Fire.
			if ( AI_Script.Detect_Flag ) { // Target is detected.
	...
	bool Cast_Ray () {
		...
		if ( Physics.Raycast ( Temp_Ray , out Temp_RaycastHit , AI_Script.Visibility_Radius , Ray_LayerMask ) ) {
			if ( Temp_RaycastHit.transform.root == AI_Script.Target_Root_Transform ) { // Cannon can directly aim the target.
				return true ;
	...
	void Fire_Process () {
		if ( AI_Script.Target_Distance < AI_Script.OpenFire_Distance && Turret_Horizontal_Script.Fire_Flag && Fire_Flag ) { // Turret is ready, and Ray hits the target.
			if ( Mathf.Abs ( Target_Angle ) < AI_Script.Fire_Angle ) { // Cannon is within the angle range.
				Cannon_Count += Time.fixedDeltaTime ;
				if ( Cannon_Count > AI_Script.Fire_Count && Cannon_Fire_Script.Reload_Flag ) { // Cannon_Count is over, and Reload is finished.
				...
				if ( AI_Script.Direct_Fire ) {
					if ( AI_Script.Near_Flag ) { // The target is within Approach_Distance.
						Wait_Count += Time.fixedDeltaTime ;
						if ( Wait_Count > 5.0f ) { // Is the target out of the angle range?
							Cannon_Count = 0.0f ;
							Wait_Count = 0.0f ;
							Random_Offset () ;
						}
	...
	void Random_Offset () { // Give the random value to the "Target_Offset". ( Do not confuse "Target_Offset" and "Offset_Angle".)
		Vector3 Temp_Offset ;
	...
	void Get_Bullet_Generator ( GameObject Temp_Object ) { // Called from "Bullet_Generator".
